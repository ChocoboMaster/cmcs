			...
			foreach(array_shift($array) as $user){
				if(strlen($user)>1){
					...
					echo "<tr><td>$op_count</td>
							<td>$user</td>
							<td><button type='button' class='btn btn-danger delOp' value='$user'><i class='icon icon-white icon-ban-circle'></i>
			...
			if($result!=""){
				...
				foreach(array_shift($array) as $user){
					if(strlen($user)>1){
						...
						echo "<tr><td>$op_count</td>";
						echo "<td>$user</td>";
						echo "<td><button type='button' class='btn btn-danger delWL' value='$user'><i class='icon icon-white icon-ban-circle'></i> Enlever</button></td></tr>";
					}
				}
